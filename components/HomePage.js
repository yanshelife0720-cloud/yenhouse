...
export default function HomePage({ data }) {
  ...
  const heroImage = c("hero_image", "");
  const aboutImage = c("about_image", "");

  return (
    <>
      {loading && (
        <div style={{ position: "fixed", inset: 0, zIndex: 9999, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "var(--cream)", animation: "fadeOut 0.8s ease 1.4s forwards" }}>
          <div style={{ animation: "drawIn 1.4s cubic-bezier(0.16,1,0.3,1)" }}><Logo size={64} /></div>
          <div style={{ marginTop: "1.5rem", fontSize: "0.7rem", letterSpacing: "0.4em", color: "var(--warm-gold)", animation: "drawIn 1.6s ease" }}>極致造就生活品質</div>
        </div>
      )}

      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 1000, padding: scrolled ? "0.9rem 2rem" : "1.4rem 2rem", background: scrolled ? "rgba(255,253,249,0.8)" : "transparent", backdropFilter: scrolled ? "blur(20px)" : "none", WebkitBackdropFilter: scrolled ? "blur(20px)" : "none", borderBottom: scrolled ? "1px solid rgba(230,220,203,0.6)" : "1px solid transparent", transition: "all 0.5s cubic-bezier(0.16,1,0.3,1)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <Logo size={36} />
        <div className="nav-links" style={{ display: "flex", gap: "2.5rem", alignItems: "center" }}>
          {[["關於燕舍", "#about"], ["服務項目", "#services"], ["案例分享", "#projects"], ["為什麼選擇燕舍", "#why"], ["聯絡我們", "#contact"]].map(([label, href]) => (
            <a key={href} href={href} style={{ fontSize: "0.9rem", letterSpacing: "0.05em", color: "var(--ink-soft)", transition: "color 0.3s" }} onMouseEnter={(e) => (e.target.style.color = "var(--warm-gold)")} onMouseLeave={(e) => (e.target.style.color = "var(--ink-soft)")}>{label}</a>
          ))}
          <a href={lineUrl} target="_blank" rel="noreferrer" className="btn-primary" style={{ padding: "0.65rem 1.6rem", fontSize: "0.85rem" }}>立即諮詢</a>
        </div>
        <button className="menu-btn" onClick={() => setMenuOpen(!menuOpen)} style={{ display: "none", background: "none", border: "none", fontSize: "1.5rem", color: "var(--ink)" }}>☰</button>
      </nav>

      {menuOpen && (
        <div className="mobile-menu glass" style={{ position: "fixed", top: 0, right: 0, bottom: 0, width: "75%", zIndex: 1001, padding: "5rem 2rem", display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      <header className="warm-glow-bg" style={{ position: "relative", minHeight: "100vh", display: "flex", alignItems: "center", overflow: "hidden" }}>
        ...
        <div ref={heroRef} style={{ position: "relative", zIndex: 2, maxWidth: 1200, margin: "0 auto", padding: "0 2rem", width: "100%" }}>
          <div className="hero-grid" style={{ display: "grid", gridTemplateColumns: "1.1fr 1fr", gap: "3rem", alignItems: "center" }}>
            <div>
              ...
              <h1 style={{ fontSize: "clamp(2.8rem, 6vw, 4.5rem)", lineHeight: 1.25, marginBottom: "1.5rem", animation: "drawIn 1.2s ease 1.5s both" }}>{c("hero_title", "極致造就生活品質")}</h1>
              <p style={{ fontSize: "1.05rem", color: "var(--ink-soft)", letterSpacing: "0.08em", marginBottom: "2.5rem", animation: "drawIn 1.4s ease 1.8s both" }}>{c("hero_subtitle", "包租代管｜空間優化｜AI空間提案｜短影音曝光")}</p>
        ...
        <div style={{ position: "absolute", bottom: "2rem", left: "50%", transform: "translateX(-50%)", fontSize: "0.7rem", letterSpacing: "0.3em", color: "var(--taupe)", animation: "float 2.5s ease-in-out infinite" }}>向下滑動 ↓</div>
      </header>
      ...
      <section id="about" className="section">
        <div className="about-grid reveal" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4rem", alignItems: "center" }}>
          ...
          <div className="glass" style={{ borderRadius: 24, overflow: "hidden", aspectRatio: "1/1", boxShadow: "var(--shadow-medium)" }}>
            {aboutImage ? (
              <img src={aboutImage} alt="關於燕舍" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
            ) : (
              <div style={{ width: "100%", height: "100%", background: "linear-gradient(145deg, #faf7f1, #e6dccb)", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--taupe)", fontSize: "0.75rem", letterSpacing: "0.25em" }}>關於燕舍圖片</div>
            )}
          </div>
        </div>
      </section>
        ...
        <div className="section" style={{ padding: "1rem 2rem" }}>
          ...
          <div className="services-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "1.5rem" }}>
            {(services.length ? services : []).map((s, i) => (
              <div key={s.id} className="glass reveal service-card" style={{ borderRadius: 20, padding: "2.5rem 1.8rem", textAlign: "center", transition: "all 0.6s cubic-bezier(0.16,1,0.3,1)", transitionDelay: `${i * 0.08}s`, cursor: "pointer" }}
      ...
      <section id="projects" className="section">
        ...
        <div className="reveal" style={{ display: "flex", gap: "0.8rem", justifyContent: "center", marginBottom: "3rem", flexWrap: "wrap" }}>
          {cats.map((cat) => (
            <button key={cat} onClick={() => setActiveCat(cat)} style={{ padding: "0.5rem 1.4rem", borderRadius: 100, fontSize: "0.85rem", letterSpacing: "0.05em", border: "1px solid " + (activeCat === cat ? "var(--warm-gold)" : "var(--beige)"), background: activeCat === cat ? "var(--warm-gold)" : "transparent", color: activeCat === cat ? "white" : "var(--ink-soft)", transition: "all 0.3s" }}>{cat}</button>
          ))}
        </div>
            ...
            <div key={p.id} className="reveal" style={{ transitionDelay: `${i * 0.08}s` }}>
              <div className="glass" style={{ borderRadius: 18, overflow: "hidden", boxShadow: "var(--shadow-soft)" }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", aspectRatio: "2/1.3" }}>
                  <div style={{ background: p.before_image ? "none" : "linear-gradient(135deg,#e6dccb,#d9cbb3)", position: "relative", overflow: "hidden" }}>
                    <span style={{ position: "absolute", top: 8, left: 8, fontSize: "0.6rem", background: "rgba(58,52,43,0.5)", color: "white", padding: "2px 8px", borderRadius: 4, zIndex: 1 }}>BEFORE</span>
                    {p.before_image ? <img src={p.before_image} alt="before" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--taupe)", fontSize: "0.7rem" }}>Before</div>}
                  </div>
                  <div style={{ background: p.after_image ? "none" : "linear-gradient(135deg,#faf7f1,#f0e9dd)", position: "relative", overflow: "hidden" }}>
                    <span style={{ position: "absolute", top: 8, right: 8, fontSize: "0.6rem", background: "var(--warm-gold)", color: "white", padding: "2px 8px", borderRadius: 4, zIndex: 1 }}>AFTER</span>
                    {p.after_image ? <img src={p.after_image} alt="after" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--taupe)", fontSize: "0.7rem" }}>After</div>}
                  </div>
                </div>
      ...
      <section id="why" className="warm-glow-bg" style={{ padding: "6rem 0" }}>
        <div className="section" style={{ padding: "1rem 2rem" }}>
          ...
          <div className="why-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "2.5rem" }}>
            {[["提升出租效率", "專業代管，提高出租率與穩定度"], ["提升空間價值", "空間優化設計，提高租金與價值"], ["AI 視覺提案", "AI 渲染分析，預見空間可行性"], ["專業整合服務", "一站式服務，省時省力更安心"], ["高品質空間規劃", "皇尚打造設計，提升生活品質"], ["品牌化出租思維", "打造品牌免費，提升物件競爭力"]].map(([title, desc], i) => (
              <div key={title} className="reveal" style={{ textAlign: "center", transitionDelay: `${i * 0.06}s` }}>
                <div style={{ fontFamily: "var(--font-display)", fontSize: "2.5rem", color: "var(--warm-gold)", opacity: 0.5, marginBottom: "0.5rem" }}>0{i + 1}</div>
        ...
        <div className="glass reveal" style={{ borderRadius: 32, padding: "5rem 2rem", position: "relative", overflow: "hidden", boxShadow: "var(--shadow-medium)" }}>
          ...
          <div style={{ position: "relative", zIndex: 2 }}>
            ...
            <div style={{ display: "flex", gap: "1rem", justifyContent: "center", flexWrap: "wrap" }}>
              <a href={lineUrl} target="_blank" rel="noreferrer" className="btn-primary" style={{ background: "linear-gradient(135deg,#06c755,#05a847)", boxShadow: "0 8px 30px rgba(6,199,85,0.3)" }}>{c("cta_btn1", "LINE 立即諮詢")}</a>
